...
# here we fetch the text8 dataset from the huggingface hub, and save it to a file
r = requests.get("https://huggingface.co/datasets/ardMLX/text8/resolve/main/text8")
with open("text8.txt", "wb") as f: f.write(r.content)
with open('text8.txt') as f: text8: str = f.read()

# this is basically a very minimal text normalizing function that just cleans up the text a bit
...
# this is the actual vocabulary building step
...
# this creates a mapping from words to ids, and vice versa, it builds a tuples with the two dictionary mappings 
...
# this is the real tokenization step, where we convert the words in the corpus to ids
...
# print(words_to_ids['kafofhio']) # will raise a KeyError
